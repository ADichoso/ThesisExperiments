...
def _resolve_dataset_root(data_root=None):
    ...
    raise RuntimeError(
        "Could not find a UGTR dataset folder. Expected a folder containing "
...
def make_dataset(data_root=None):
    ...
    images = [os.path.join(image_root, f) for f in os.listdir(image_root) if f.endswith('.jpg') or f.endswith('.png')]
    gts = [os.path.join(gt_root, f) for f in os.listdir(gt_root) if f.endswith('.png')]
    edges = [os.path.join(edge_root, f) for f in os.listdir(image_root) if f.endswith('.png')]
    image_label_list = []
...
class SemData(Dataset):
    def __init__(self, split='train', dataset=None, data_root=None, transform=None):
        print(data_root)
        self.split = split
        self.data_list = make_dataset(data_root)
