...
for epoch in range(1, (opt.epoch+1)):
    ...
    for i, pack in enumerate(train_loader, start=1):
        for rate in size_rates:
            ...
            fix_pred,cod_pred2 = generator.forward(images)
            fix_loss = structure_loss(fix_pred, fixs)
            cod_loss2 = structure_loss(cod_pred2, gts)
            final_loss = fix_loss + cod_loss2
            final_loss.backward()
            ...
            if rate == 1:
                ...
                loss_record2.update(cod_loss2.data, opt.batchsize)

        if i % 10 == 0 or i == total_step:
            print('{} Epoch [{:03d}/{:03d}], Step [{:04d}/{:04d}], Gen Loss: {:.4f}, fix Loss: {:.4f}'.
                  format(datetime.now(), epoch, opt.epoch, i, total_step, loss_record.show(), loss_record2.show()))
