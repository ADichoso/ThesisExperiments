...
class MFA(nn.Module):
    def __init__(self, in_dim):
        ...
        self.layer_cat1 = nn.Sequential(nn.Conv2d(in_dim * 2, in_dim, kernel_size=3, stride=1, padding=1),
...
class PopNet(nn.Module):
    def __init__(self, channel=32, ind=50):
        ...
        ###############################################
        #  𝐒𝐞𝐠𝐦𝐞𝐧𝐭𝐚𝐭𝐢𝐨𝐧 𝐰𝐢𝐭𝐡 𝐂𝐨𝐧𝐭𝐚𝐜𝐭 𝐒𝐮𝐫𝐟𝐚𝐜𝐞 #
        ###############################################
    ...
    def forward(self, imgs, depths):
        ...
        t_out     = self.upsample_4(self.ful_conv_out1(x_ful_02))

        return rgb_out, dep_out, ful_out, t_out.sigmoid(), x0_2
