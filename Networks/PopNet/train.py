...
# Load data
print('Loading data...')
train_loader = get_loader(train_image_root, train_gt_root, train_depth_root,
...
# Logging
logging.basicConfig(
    filename='./Logs/Train/PopNet_log.txt',
    ...
    try:
        for i, (images, gts, depths) in enumerate(train_loader, start=1):
            ...
            if i % 50 == 0 or i == total_step or i == 1:
                print('{} Epoch [{:03d}/{:03d}], Step [{:04d}/{:04d}], Loss1: {:.4f} Loss2: {:.4f} Loss3: {:.4f}'.format(
                    datetime.now(), epoch, opt.epoch, i, total_step, loss1.data, loss2.data, loss3.data))
                logging.info(
                    '#TRAIN#:Epoch [{:03d}/{:03d}], Step [{:04d}/{:04d}], Loss1: {:.4f} Loss2: {:.4f} Loss3: {:.4f}'.format(
                        epoch, opt.epoch, i, total_step, loss1.data, loss2.data, loss3.data))
        ...
        if not os.path.exists(save_path):
            os.makedirs(save_path)
        torch.save(model.state_dict(), save_path + 'PopNet_epoch_{}.pth'.format(epoch + 1))
        print('Saved checkpoints successfully!')
        raise
