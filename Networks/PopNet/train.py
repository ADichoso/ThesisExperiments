...
logging.basicConfig(filename='./Logs/Train/PopNet_log.txt', format='[%(asctime)s-%(filename)s-%(levelname)s:%(message)s]',
...
def train(train_loader, model, optimizer, epoch, save_path):  
    ...
    try:
        ...
        if (epoch) % 5 == 0:
            torch.save(model.state_dict(), save_path + 'PopNet_epoch_{}.pth'.format(epoch))  # 保留参数模型
