...
class DataLoadPreprocess(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.mode == 'train':
            ...
            if self.mode == 'online_eval':
                sample = {'image': image, 'depth': depth_gt, 'focal': focal, 'has_valid_depth': has_valid_depth, 'orig_size': (orig_w, orig_h)}
            else:
                sample = {'image': image, 'focal': focal, 'orig_size': (orig_w, orig_h)}
...
class ToTensor(object):
    ...
    def __call__(self, sample):
        ...
        if self.mode == 'test':
            return {'image': image, 'focal': focal, 'orig_size': sample['orig_size']}
